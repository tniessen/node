...
function getAuthTagLength(options) {
  ...
  if (options && (authTagLength = options.authTagLength) != null) {
    ...
    if (~~authTagLength !== authTagLength || authTagLength < 0) {
      throw new errors.TypeError('ERR_INVALID_OPT_VALUE', 'authTagLength',
                                 authTagLength);
    }
...
function Cipher(cipher, password, options) {
  ...
  const authTagLength = getAuthTagLength(options);
